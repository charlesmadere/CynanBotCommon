...
class TwitchApiService(TwitchApiServiceInterface):
    ...
    async def createEventSubSubscription(
        ...
        if not utils.isValidStr(twitchAppAccessToken):
        ...
        self.__timber.log('TwitchApiService', f'Creating EventSub subscription... ({twitchAppAccessToken=}) ({eventSubRequest=})')
        ...
        try:
            response = await clientSession.post(
                ...
                headers = {
                    'Authorization': f'Bearer {twitchAppAccessToken}',
                    'Client-Id': twitchClientId
                },
                json = eventSubRequest.toJson()
            )
        except GenericNetworkException as e:
            self.__timber.log('TwitchApiService', f'Encountered network error when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {e}', e, traceback.format_exc())
            raise GenericNetworkException(f'TwitchApiService encountered network error when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {e}')
        ...
        if not utils.hasItems(jsonResponse):
            self.__timber.log('TwitchApiService', f'Received a null/empty JSON resposne when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchJsonException(f'TwitchApiService received a null/empty JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
        elif responseStatusCode != 202:
            self.__timber.log('TwitchApiService', f'Encountered non-202 HTTP status code ({responseStatusCode}) when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchStatusCodeException(f'TwitchApiService encountered non-202 HTTP status code ({responseStatusCode}) when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')

        data: Optional[List[Dict[str, Any]]] = jsonResponse.get('data')
        if not utils.hasItems(data):
            self.__timber.log('TwitchApiService', f'Received a null/empty \"data\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchJsonException(f'TwitchApiService received a null/empty \"data\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')

        dataJson: Dict[str, Any] = data[0]
        if not utils.hasItems(dataJson):
            self.__timber.log('TwitchApiService', f'Received a null/empty first \"data\" field element in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchJsonException(f'TwitchApiService received a null/empty first \"data\" field element in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
        ...
        if not utils.hasItems(conditionJson):
            self.__timber.log('TwitchApiService', f'Received a null/empty \"condition\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchJsonException(f'TwitchApiService received a null/empty \"condition\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
        ...
        if not utils.hasItems(transportJson):
            self.__timber.log('TwitchApiService', f'Received a null/empty \"transport\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
            raise TwitchJsonException(f'TwitchApiService received a null/empty \"transport\" field in the JSON response when creating EventSub subscription (twitchAccessToken=\"{twitchAppAccessToken}\") (eventSubRequest=\"{eventSubRequest}\"): {jsonResponse}')
